...
var _ = Describe("KubernetesListTopRestartedPods Tool", func() {
	var (
		tool       *tools.KubernetesListTopRestartedPods
	...
	BeforeEach(func() {
		...
		tool = tools.NewKubernetesListTopRestartedPods(mockSysdig)
		mcpServer = server.NewMCPServer("test", "test")
		tool.RegisterInServer(mcpServer)
	})

	It("should register successfully in the server", func() {
		Expect(mcpServer.GetTool("kubernetes_list_top_restarted_pods")).NotTo(BeNil())
	})
			...
			Entry(nil,
				...
				mcp.CallToolRequest{
					Params: mcp.CallToolParams{
						Name:      "kubernetes_list_top_restarted_pods",
