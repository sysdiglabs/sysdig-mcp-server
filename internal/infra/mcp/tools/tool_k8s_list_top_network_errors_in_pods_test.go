...
var _ = Describe("KubernetesListTopNetworkErrorsInPods Tool", func() {
	var (
		tool       *tools.K8sListTopNetworkErrorsInPods
	...
	BeforeEach(func() {
		...
		tool = tools.NewK8sListTopNetworkErrorsInPods(mockSysdig, mockClock)
	...
	It("should register successfully in the server", func() {
		Expect(mcpServer.GetTool("k8s_list_top_network_errors_in_pods")).NotTo(BeNil())
	})
	...
	When("listing top network errors", func() {
		DescribeTable("it succeeds", func(ctx context.Context, toolName string, request mcp.CallToolRequest, expectedParamsRequested sysdig.GetQueryV1Params) {
			...
			Entry("default params (legacy path)",
				"k8s_list_top_network_errors_in_pods",
				mcp.CallToolRequest{
					Params: mcp.CallToolParams{
						Name:      "k8s_list_top_network_errors_in_pods",
			...
			Entry("windowed path via start/end",
				...
				mergeLimit(newWindowedQueryParams(
					`topk(20,sum(sum_over_time(sysdig_container_net_error_count{}[3600s])) by (kube_cluster_name, kube_namespace_name, kube_workload_type, kube_workload_name, kube_pod_name)) / 3600`,
		...
		It("returns error for invalid interval", func() {
			serverTool := mcpServer.GetTool("k8s_list_top_network_errors_in_pods")
