...
var _ = Describe("KubernetesListUnderutilizedPodsMemoryQuota Tool", func() {
	var (
		tool       *tools.KubernetesListUnderutilizedPodsMemoryQuota
	...
	BeforeEach(func() {
		...
		tool = tools.NewKubernetesListUnderutilizedPodsMemoryQuota(mockSysdig)
		mcpServer = server.NewMCPServer("test", "test")
		tool.RegisterInServer(mcpServer)
	})

	It("should register successfully in the server", func() {
		Expect(mcpServer.GetTool("kubernetes_list_underutilized_pods_memory_quota")).NotTo(BeNil())
	})
			...
			Entry(nil,
				...
				mcp.CallToolRequest{
					Params: mcp.CallToolParams{
						Name:      "kubernetes_list_underutilized_pods_memory_quota",
