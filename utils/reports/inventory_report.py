...
def list_all_resources(filter_exp: str = 'platform in ("GCP")') -> dd.DataFrame:
    ...
    try:
        resources = inventory.tool_list_resources(filter_exp=filter_exp, page_number=1, page_size=1000)
        ...
        while resources.get("results", {}).get("page", {}).get("next"):
            ...
            resources = inventory.tool_list_resources(filter_exp=filter_exp, page_number=next_page, page_size=1000)
            df = dd.concat(
