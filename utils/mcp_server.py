...
def run_http():
    ...
    mcp_app = mcp.http_app(path="/mcp", transport=os.environ.get("MCP_TRANSPORT", app_config["mcp"]["transport"]).lower(), middleware=middlewares)
    ...
    uvicorn.run(app, port=app_config["app"]["port"], host=app_config["app"]["host"], log_level=os.environ.get("LOGLEVEL", app_config["app"]["log_level"]).lower())
    return 0
...
def add_tools(mcp: FastMCP) -> None:
    ...
    mcp.add_tool(events_feed_tools.tool_get_event_info, 
                 name="get_event_info", 
                 description="Retrieve detailed information for a specific security event by its ID")
    mcp.add_tool(events_feed_tools.tool_list_runtime_events, 
    ...
    mcp.add_prompt(events_feed_tools.investigate_event_prompt,
    ...
    mcp.add_tool(events_feed_tools.tool_get_event_process_tree,
                    name="get_event_process_tree",
                    description="Retrieve the process tree for a specific security event by its ID. Not every event has a process tree, so this may return an empty tree." 
                )
    
    # Register the Sysdig Inventory tools
    log.info("Adding Sysdig Inventory Tools...")
    inventory_tools = InventoryTools()
    mcp.add_tool(inventory_tools.tool_list_resources, 
                 name="list_resources", 
                 description="List inventory resources based on a Sysdig Secure query filter expression with optional pagination. Example filters: not isExposed exists; category in (\"IAM\") and isExposed exists; category in (\"IAM\",\"Audit & Monitoring\"); vuln.hasFix exists and vuln.hasExploit exists and isExposed exists and package.inUse exists and validatedExposure exists and control.failed in (\"Contains AI Package\");")
    mcp.add_tool(inventory_tools.tool_get_resource,
                name="get_resource",
    ...
    mcp.add_tool(sysdig_sage_tools.tool_sysdig_sage,
                name="sysdig_sysql_sage_query",
                description="Query Sysdig Sage to generate a SysQL query based on a natural language question, execute it against the Sysdig API, and return the results.",
                )
